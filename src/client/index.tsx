...
function scheduleLabel(task: AutomationTaskView, locale: string, t: typeof translate): string {
  if (task.schedule.kind === 'once') return `${t('once')} · ${new Date(task.schedule.fireAt).toLocaleString(locale)}`
...
function triggerLabel(trigger: string, t: typeof translate): string {
  return trigger === 'manual' ? t('triggerManual') : t('triggerScheduled')
}
    ...
    >
      ...
      >
        <header style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 16 }}>
          <h2 style={{ margin: 0, flex: 1 }}>{t('automations')}</h2>
          <button type="button" style={buttonStyle} onClick={() => void refresh()} disabled={loading}>{t('refresh')}</button>
          <button type="button" style={buttonStyle} onClick={() => setPanelOpen(false)}>{t('close')}</button>
        </header>

        <div role="alert" style={{ border: '1px solid #d1242f', borderRadius: 10, padding: 12, marginBottom: 16, background: 'rgba(209,36,47,.08)' }}>
          <strong>{t('fullAccessTitle')}</strong> {t('fullAccessMessage')}
        </div>
            ...
            return (
              <article key={task.id} style={{ border: '1px solid var(--dsh-border, #d0d0d0)', borderRadius: 12, padding: 16 }}>
                <div style={{ display: 'flex', alignItems: 'baseline', gap: 8 }}>
                  ...
                  <strong style={{ color: statusColor(displayStatus) }}>{statusLabel(displayStatus, t)}</strong>
                </div>
                <p style={{ margin: '8px 0 4px' }}>{scheduleLabel(task, locale, t)}</p>
                <p style={{ margin: '4px 0', opacity: .72 }}>{t('next')}: {task.nextRunAt === null ? '—' : new Date(task.nextRunAt).toLocaleString(locale)}</p>
                <p style={{ margin: '4px 0', opacity: .72 }}>{t('workspace')}: {task.execution.cwd}</p>
                <p style={{ margin: '4px 0 12px', fontFamily: 'monospace', fontSize: 12 }}>{task.id}</p>

                <div style={{ display: 'flex', flexWrap: 'wrap', gap: 8 }}>
                  <button type="button" style={buttonStyle} disabled={busy} onClick={() => void act(`/tasks/${encodeURIComponent(task.id)}/run`, { method: 'POST' })}>{t('runNow')}</button>
                  {task.status === 'active' && (
                    <button type="button" style={buttonStyle} onClick={() => void act(`/tasks/${encodeURIComponent(task.id)}/pause`, { method: 'POST' })}>{t('pause')}</button>
                  )}
                  {task.status === 'paused' && (
                    <>
                      <button type="button" style={buttonStyle} onClick={() => void act(`/tasks/${encodeURIComponent(task.id)}/resume`, { method: 'POST', body: JSON.stringify({ runNow: false }) })}>{t('resume')}</button>
                      <button type="button" style={buttonStyle} disabled={busy} onClick={() => void act(`/tasks/${encodeURIComponent(task.id)}/resume`, { method: 'POST', body: JSON.stringify({ runNow: true }) })}>{t('resumeAndRun')}</button>
                    </>
                  )}
                  {latestSession !== undefined && (
                    <button type="button" style={buttonStyle} onClick={() => ctx.sessions.open(latestSession as SessionId)}>{t('openLatestSession')}</button>
                  )}
                  <button
                    type="button"
                ...
                {task.runs.length > 0 && (
                  <details style={{ marginTop: 14 }}>
                    <summary>{t('recentRuns', { count: task.runs.length })}</summary>
                    <ol style={{ paddingLeft: 20 }}>
                      {[...task.runs].reverse().map((run) => (
                        <li key={run.id} style={{ margin: '8px 0' }}>
                          <span style={{ color: statusColor(run.status) }}>{statusLabel(run.status, t)}</span>
                          {' · '}{triggerLabel(run.trigger, t)}{' · '}{new Date(run.startedAt ?? run.enqueuedAt).toLocaleString(locale)}
                          {run.sessionId !== undefined && (
                            <button type="button" style={{ ...buttonStyle, marginLeft: 8, padding: '2px 7px' }} onClick={() => ctx.sessions.open(run.sessionId as SessionId)}>{t('open')}</button>
                          )}
