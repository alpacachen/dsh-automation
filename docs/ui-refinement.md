...
Build first, then run `node scripts/test-ui-browser.mjs` with Playwright available. Optional environment variables: `PLAYWRIGHT_MODULE` (absolute path, `./`-relative path from the working directory, `file:` URL, or package specifier; defaults to `playwright`), `CHROME_PATH` (browser executable), `DSH_TEST_URL` (default existing 3080), `DSH_TEST_COOKIE` (local authenticated GUI cookie, supplied in the environment only), and `UI_SCREENSHOT_DIR` (defaults to `automation-ui-review` under Node's `os.tmpdir()`). Do not commit authentication values. No replacement server is started.

Run `node scripts/check-tokens.mjs` from the repository root. `DSH_THEME_FILE` explicitly selects the Host theme bundle or CSS file (relative paths use the working directory), taking precedence over discovery. Without it, the script resolves the installed `@deepseek-ai/dsh-client-ui-theme/client`, then checks the npm `_npx` cache one level deep in sorted order. The cache uses `npm_config_cache` when set; otherwise `~/.npm` on macOS/Linux or `%LOCALAPPDATA%/npm-cache` on Windows. If several Host versions are cached, use `DSH_THEME_FILE` to select the one being tested.
