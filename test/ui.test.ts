...
test('Automation UI uses DSH tokens and accessible interaction states', async () => {
  ...
  assert.match(client, /selectedSkills/)
  assert.match(client, /modelFailures/)
  assert.match(client, /approvalAskWarning/)
  assert.match(client, /setPermissionConfirmed\(false\)/)
  assert.match(client, /hostDefault/)
  assert.match(client, /executionDestination/)
  assert.match(client, /executionFresh/)
  assert.match(client, /executionPinned/)
  assert.doesNotMatch(client, /<b>Execution destination/)
  assert.match(client, /guidedCreationPrompt/)
  assert.match(client, /automation-create-button/)
