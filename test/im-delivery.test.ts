...
function service() {
  ...
  return {
    sends,
    async listBots() {
      return [{ botId: destination.botId, channel: 'telegram', token: 'must-not-leak' }]
    },
    async listTargets() {
      return [{ targetId: destination.targetId, name: 'My phone', kind: 'chat', route: { privateId: 'must-not-leak' } }]
    },
...
test('optional Cordis delivery service discovers only public bot/target labels', async (t) => {
  ...
  assert.deepEqual(await deliveryOptions(ctx), {
    available: true,
    bots: [{ botId: 'bot-test', channel: 'telegram' }],
    targets: [],
  })
  assert.deepEqual((await deliveryOptions(ctx, destination.botId)).targets, [
    { targetId: 'daily-report', name: 'My phone', kind: 'chat' },
  ])
...
test('direct send preserves full Markdown output and passes format and signal without a Session or sync', async (t) => {
  const ctx = new Context()
  t.after(() => ctx.fiber.dispose())
  const im = service()
  ctx.provide('dshIm', im)
  const signal = new AbortController().signal
  const output = `# Daily report\n\n**Highlights**\n- [Details](https://example.com/report)\n\n\`\`\`ts\nconst ready = true\n\`\`\`\n${'full reply '.repeat(120)}`
  ...
  assert.deepEqual(im.sends[0]?.[3], { signal, format: 'markdown' })
...
test('missing/offline/rejected/canceled service calls surface without hidden retry', async (t) => {
  ...
  await assert.rejects(
    sendAutomationResult(ctx, { ...task, delivery: undefined }, run, outcome, signal),
    /No message delivery/,
  )
